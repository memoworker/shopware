...
class Migration1752750086AddIndexToOrderLineItemCreateAndUpdateTest extends TestCase
{
    ...
    public function testMigration(): void
    {
        ...
        static::assertArrayHasKey('idx.order_line_item_created_at', $existingIndexes);
        static::assertArrayHasKey('idx.order_line_item_updated_at', $existingIndexes);
    }
    ...
    private function rollback(): void
    {
        ...
        if (isset($existingIndexes['idx.order_line_item_created_at'])) {
            $this->connection->executeStatement('DROP INDEX `idx.order_line_item_created_at` ON `order_line_item`');
        }

        if (isset($existingIndexes['idx.order_line_item_updated_at'])) {
            $this->connection->executeStatement('DROP INDEX `idx.order_line_item_updated_at` ON `order_line_item`');
        }
    }
}
