...
class Migration1752750171AddIndexToOrderAddressCreateAndUpdateTest extends TestCase
{
    ...
    public function testMigration(): void
    {
        ...
        static::assertArrayHasKey('idx.order_address_created_at', $existingIndexes);
        static::assertArrayHasKey('idx.order_address_updated_at', $existingIndexes);
    }
    ...
    private function rollback(): void
    {
        ...
        if (isset($existingIndexes['idx.order_address_created_at'])) {
            $this->connection->executeStatement('DROP INDEX `idx.order_address_created_at` ON `order_address`');
        }

        if (isset($existingIndexes['idx.order_address_updated_at'])) {
            $this->connection->executeStatement('DROP INDEX `idx.order_address_updated_at` ON `order_address`');
        }
    }
}
