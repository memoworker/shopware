...
{
    private const CUSTOM_VENDOR_CUSTOM_TEST_ROUTE = 'customVendor.custom_test_route';
    ...
    protected function setUp(): void
    {
        ...
        $this->router = new Router($this->symfonyRouterMock, $this->requestStackMock, [self::CUSTOM_VENDOR_CUSTOM_TEST_ROUTE]);
    }
    ...
    public function testGeneratePathInfo(): void
    {
        ...
        $result = $this->router->generate('frontend.test_route', [], Router::PATH_INFO);
    ...
    public function testGenerateWithCustomRouteIsAllowedAsStorefrontRoute(): void
    {
        $this->symfonyRouterMock
            ->expects($this->once())
            ->method('generate')
            ->with(self::CUSTOM_VENDOR_CUSTOM_TEST_ROUTE, [])
            ->willReturn('/base-path/' . self::CUSTOM_VENDOR_CUSTOM_TEST_ROUTE);

        $this->requestStackMock
            ->expects($this->exactly(2))
            ->method('getMainRequest');

        $result = $this->router->generate(self::CUSTOM_VENDOR_CUSTOM_TEST_ROUTE);

        static::assertSame('/base-path/' . self::CUSTOM_VENDOR_CUSTOM_TEST_ROUTE, $result);
    }

    public function testGenerateWithAbsoluteHttpsUrl(): void
    {
        $routeName = 'frontend.test_route';
    ...
    public function testGenerateWithAbsolutePath(): void
    {
        ...
        $result = $this->router->generate('frontend.test_route');
